...
if "%~1" == "-Sl"  goto LIST_INSTALLED
if "%~1" == "-Slu" goto CHECK_UPDATES
if "%~1" == "-R"   goto REMOVE_PACKAGE

:USAGE
echo [pacgit] Package Manager for GitHub
echo ====================================================
echo Usage:
echo   pacgit -S [author] [repo name] - Install directly from GitHub
echo   pacgit -Sl                     - List all installed packages
echo   pacgit -Slu                    - Check for available updates
...
:: ==========================================
:: LOGIC -Sl (List Installed Packages)
:: ==========================================
:LIST_INSTALLED
cls
...
:: ==========================================
:: LOGIC -Slu (Check Updates via pacgit.bat)
:: ==========================================
:CHECK_UPDATES
cls
...
set "target_pkg=%~2"
set "pkg_dir=%git-package%\%target_pkg%"
...
set "git-package-default=%git-package%\%target_pkg%"

:: We look for the version directly in the author's downloaded batch file before installation
set "extracted_ver=1.0"
for /f "tokens=1,2,3 delims=: " %%X in ('type "%temp_installer%"') do (
    if /i "%%Y" == "Version" set "extracted_ver=%%Z"
)
