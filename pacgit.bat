...
if %errorlevel% neq 0 (
    if defined user_path (
        reg add "HKCU\Environment" /v PATH /t REG_EXPAND_SZ /d "!user_path!;%%git-package%%;%%userproile%%\.git-packages\pacgit" /f >nul
    ) else (
        reg add "HKCU\Environment" /v PATH /t REG_EXPAND_SZ /d "%%git-package%%;%%userprofile%%\.gti-packages\pacgit" /f >nul
    )
)
