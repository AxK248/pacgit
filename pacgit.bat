...
if %errorlevel% neq 0 (
    reg add "HKCU\Environment" /v PATH /t REG_EXPAND_SZ /d "!user_path!;%%git-package%%;%%pacgit%%" /f >nul
)
...
reg add "HKCU\Environment" /v pacgit /t REG_EXPAND_SZ /d "^%git-package^%\pacgit"
